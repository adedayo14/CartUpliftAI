...
export async function getBundleInsights({
  ...
  try {
    ...
    if (!response.ok) {
      console.warn("Bundle insights GraphQL error", await safeParseError(response));
      return { bundles: [], opportunities: [], totalOrdersConsidered: 0 };
    }
    ...
    if (payload?.errors?.length) {
      console.warn("Bundle insights GraphQL errors", payload.errors);
      return { bundles: [], opportunities: [], totalOrdersConsidered: 0 };
    }

    const orderEdges: any[] = payload?.data?.orders?.edges ?? [];

    const pairMap = new Map<string, BundleAggregate>();
    let totalOrdersConsidered = 0;

    for (const edge of orderEdges) {
      const lineItems: any[] = edge?.node?.lineItems?.edges ?? [];
    ...
    const bundles: BundleSummary[] = aggregates.map((aggregate, index) => {
      ...
      return {
        ...
        status: aggregate.orderCount >= 3 ? "active" : "draft",
        type: "ml",
    ...
    console.error("Bundle insights error", error);
    return { bundles: [], opportunities: [], totalOrdersConsidered: 0 };
  }
}
