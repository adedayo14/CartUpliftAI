...
export async function getDataQualityMetrics(shop: string): Promise<DataQualityMetrics> {
  try {
    ...
    if (!response.ok) {
      console.warn('Failed to fetch order count from Shopify');
    ...
    const orderData: any = await orderCountResponse.json();
...
export async function trackMLRecommendation(params: {
  ...
  try {
    ...
    console.error('Error tracking ML recommendation:', error);
...
export async function getMLUserProfile(params: {
  ...
  try {
    ...
    console.error('Error fetching ML user profile:', error);
    return null;
  }
}
...
export async function updateMLUserProfile(params: {
  ...
  try {
    ...
    console.error('Error updating ML user profile:', error);
    return { success: false };
  }
}
