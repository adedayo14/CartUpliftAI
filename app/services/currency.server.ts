...
export async function getShopCurrency(shop: string): Promise<{ code: string; format: string }> {
  ...
  try {
    ...
    console.warn('[getShopCurrency] Database lookup failed:', error);
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      try {
        ...
        console.warn('[getShopCurrency] Failed to save to database:', dbError);
      }
    ...
    console.error('[getShopCurrency] Failed to fetch from Shopify:', error);
  }

  // Final fallback - return USD but log warning
  console.warn(`[getShopCurrency] Using USD fallback for shop: ${shop}`);
