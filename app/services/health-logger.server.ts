...
export class HealthLogger {
  ...
  private errors: Array<{ message: string; details?: any }> = [];
   ...
   */
  async logError(error: Error | string, details?: any) {
  ...
  async success(result: HealthLogResult = {}) {
    ...
    try {
      await (prisma as any).mLSystemHealth.update({
...
export async function startHealthLog(
  ...
  const health = await (prisma as any).mLSystemHealth.create({
    data: {
      shop,
      jobType,
      status: 'running',
...
export async function getRecentHealthLogs(shop: string, limit = 50) {
  return await (prisma as any).mLSystemHealth.findMany({
    where: { shop },
...
export async function getHealthSummary(shop: string, days = 7) {
  ...
  const logs = await (prisma as any).mLSystemHealth.findMany({
    where: {
      shop,
      createdAt: { gte: since }
    },
  ...
  const summary = {
    ...
    successfulRuns: logs.filter((l: any) => l.status === 'success').length,
    failedRuns: logs.filter((l: any) => l.status === 'failed').length,
    partialRuns: logs.filter((l: any) => l.status === 'partial').length,
    avgDurationMs: logs.reduce((sum: number, l: any) => sum + (l.durationMs || 0), 0) / Math.max(logs.length, 1),
    totalErrors: logs.reduce((sum: number, l: any) => sum + l.errorCount, 0),
