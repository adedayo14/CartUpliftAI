...
export const action = withAuth(async ({ request }: ActionFunctionArgs) => {
  try {
    ...
    return json({ error: 'Failed to load bundle data' }, { status: 500 });
  }
});
...
export async function trackBundleAnalytics({ request }: ActionFunctionArgs) {
  try {
    ...
    return json({ error: 'Failed to track bundle analytics' }, { status: 500 });
  }
}
...
async function storeBundleAnalytics(analytics: any) {
